...
*Note*: this is a fork that is used to create NPM packages that can be used in other projects. It is the same as [the base repository](https://github.com/Dash-Industry-Forum/dash.js) with four changes:

* The package is renamed to `dashjs-npm` because `dashjs` already exists on NPM (and is not updated)
* The `main` file is changed to `dist/dash.all.debug.js` as this is the browserified version which can be used with other projects
* The `package.json` file contains a `files` section for the files to publish to NPM
* The `dist/*` files are gone from the repository. They make no sense anyway. The `*.map` files contain local paths and the other files in `dist` are generated by `npm version` / `npm publish`
